...
if "anomaly" in df.columns:
    ...
    summary_lines.append(f"Anomalías detectadas: {anomalies} / {total} ({percent_anomalies:.2f}%)")
    ...
    top_5.to_csv("results/top_5_anomalous.csv", index=False)
else:
    print("No se encontró la columna 'anomaly'. ¿Ya ejecutaste anomaly_detection.py?")
    summary_lines.append("No se encontró la columna 'anomaly'.")
with open("results/metrics_summary.txt", "w") as f:
    f.write("\n".join(summary_lines))
print("Resumen exportado a: results/metrics_summary.txt")